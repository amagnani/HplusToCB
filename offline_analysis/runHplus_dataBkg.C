...
double PtBins[] = {20, 30, 40, 50, 60, 70, 80, 100, 120, 160, 210, 260, 320, 400, 500, 600, 800};
 ...
 SFb_errors->SetBinContent( 17,2*0.0596716);
    ...
    sprintf(filename,"init/hplus_DY.ini");
...
if (flag !=1){  //not for data!
  ...
  //loading output file
   ...
   //sprintf(histofile,"output/HplusHistos_9b_%s.root",outputTag);
   sprintf(histofile,"output/PIPPO_%s.root",outputTag);
   TFile *fHistos;
 ...
 sprintf(ext9b, "9b");
      ...
      gROOT->ProcessLine(".L analyze_dataBkg.C");

   //semmai sostituire con Step0
// 	TTree *t4 = (TTree*) ch_analyzeStart;
// 	analyze_dataBkg *t = new analyze_dataBkg(t4);
...
//   
       ...
       analyze_dataBkg *rr = new analyze_dataBkg(rr4);
  ...
  cout<<"Step8b done"<<endl;
      ...
      gROOT->ProcessLine(".L analyze_dataBkg_kinfit.C");
    TTree *rr4 = (TTree*) ch_analyzeStep9b;
    analyze_dataBkg_kinfit *rrk = new analyze_dataBkg_kinfit(rr4);
