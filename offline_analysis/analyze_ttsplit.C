#define analyze_ttsplit_cxx
#include "analyze_ttsplit.h"
#include <TH2.h>
#include <TH1F.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void analyze_ttsplit::Loop(TFile *fHistos, TH2D *h2_EffMapB, TH2D * h2_EffMapC, TH2D *h2_EffMapUDSG,TH1D *SFb_errors, char *ext, int flag)
{
time_t start= time(0);
...
for(int e=0;e<9;e++){
  for(int f=0;f<5;f++){
     ...
     sprintf(name_hMUrelIso    , "Reco_hMUrelIso%s_sys%i_%i",ext,e,f);
...
//      Root > .L analyze_ttsplit.C
//      Root > analyze_ttsplit t
...
if (!usualSel) continue;
 ...
 //secondo me questa roba è sbagliata		 
	 if(MUeta!=0){
      ...
      b=1;b_up=1;b_down=1;b1_up=1;b1_down=1; t=1;t_up=1;t_down=1;
   ...
   // loop over jets
	...
	P[3]=w		*m	*b1_up	*t;		//muon up
	P[4]=w		*m	*b1_down*t;		//muon down
