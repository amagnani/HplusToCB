...
TH2D *h2_EffMapUDSG;   
TH1D *SFb_errors;
TH1F *TTmassup_weight;   
TH1F *TTmassdown_weight ;
TH1F *TTmatchup_weight;  
TH1F *TTmatchdown_weight;
TH1F *TTscaleup_weight ; 
TH1F *TTscaledown_weight;

using namespace std;


void runHplus_MCsignal()
{
double PtBins[] = {20, 30, 40, 50, 60, 70, 80, 100, 120, 160, 210, 260, 320, 400, 500, 600, 800};
 ...
 SFb_errors->SetBinContent( 17,2*0.0596716);
    ...
    sprintf(filename,"init/hplus_H160.ini");
...
if (flag !=1){  //not for data!
	...
	char sysstring[30];
	sprintf(sysstring,"efficiency/signalSys.root");
	TFile *TTin= new TFile(sysstring,"READ");
	TTmassup_weight    = (TH1F*)TTin->Get("TTmassup_weight");
	TTmassdown_weight  = (TH1F*)TTin->Get("TTmassdown_weight");
	TTmatchup_weight   = (TH1F*)TTin->Get("TTmatchup_weight");
	TTmatchdown_weight = (TH1F*)TTin->Get("TTmatchdown_weight");
	TTscaleup_weight   = (TH1F*)TTin->Get("TTscaleup_weight");
	TTscaledown_weight = (TH1F*)TTin->Get("TTscaledown_weight");
 }  
 ...
 // double Nw=NTOT/double(Ninit);
  ...
  //loading output file
   ...
   sprintf(histofile,"output/HplusHistos_%s.root",outputTag);
   TFile *fHistos;
 ...
 sprintf(ext9b, "9b");
       ...
       gROOT->ProcessLine(".L analyze_MCsignal.C");
    //semmai sostituire con Step0
      TTree *t4 = (TTree*) ch_analyzeStart;
      analyze_MCsignal *t = new analyze_MCsignal(t4);
      t->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG, TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight, SFb_errors, extS ,flag);
    
      cout<<"Start done"<<endl;
     
       TTree *rr4 = (TTree*) ch_analyzeStep4;
       analyze_MCsignal *rr = new analyze_MCsignal(rr4);
       rr->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG, TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight,SFb_errors, ext4 ,flag );
     ...
     cout<<"Step4 done"<<endl;
       ...
       analyze_MCsignal *rr = new analyze_MCsignal(rr4);
       rr->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG, TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight, SFb_errors, ext6 ,flag );
    ...
    cout<<"Step6 done"<<endl;
       ...
       analyze_MCsignal *rr = new analyze_MCsignal(rr4);
       rr->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG,TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight,SFb_errors, ext8a,flag );
   ...
   cout<<"Step8a done"<<endl;
	...
	analyze_MCsignal *rr = new analyze_MCsignal(rr4);
	rr->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG,TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight,SFb_errors, ext8b ,flag );
   
   cout<<"Step8b done"<<endl;
   
     gROOT->ProcessLine(".L analyze_MCsignal_kinfit.C");

    TTree *rr4 = (TTree*) ch_analyzeStep9b;
    analyze_MCsignal_kinfit *rrk = new analyze_MCsignal_kinfit(rr4);
    rrk->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG,TTmassup_weight, TTmassdown_weight, TTmatchup_weight , TTmatchdown_weight,
      TTscaleup_weight, TTscaledown_weight,SFb_errors, ext9b ,flag );
