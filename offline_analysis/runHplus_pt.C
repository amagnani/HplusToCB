...
double PtBins[] = {20, 30, 40, 50, 60, 70, 80, 100, 120, 160, 210, 260, 320, 400, 500, 600, 800};
 ...
 SFb_errors->SetBinContent( 17,2*0.0596716);
    ...
    sprintf(filename,"init/hplus_H120.ini");
...
if (flag !=1){  //not for data!
 ...
 // double Nw=NTOT/double(Ninit);
  ...
  //loading output file
   ...
   sprintf(histofile,"output/pt_%s.root",outputTag);
   TFile *fHistos;
 ...
 sprintf(ext9b, "9b");
       ...
       gROOT->ProcessLine(".L analyze_pt.C");
    //semmai sostituire con Step0
      
       TTree *rr4 = (TTree*) ch_analyzeStart;
       analyze_pt *rr = new analyze_pt(rr4);
       rr->Loop(fHistos, h2_EffMapB, h2_EffMapC, h2_EffMapUDSG,SFb_errors, extS ,flag );
